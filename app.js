...
app.get("/home",function(req,res){
	...
	app.get("/Shirts",function(req,res){
        ...
        mysql.query('select a.SKU, b.Product_ID, a.Name, a.Price, b.Available_Qty, c.Attr_Name, c.Attr_Value , d.Image_Path  from catalog a, product b, product_attribute c , product_image d  where a.sku=b.sku and b.product_id=c.product_id and  b.product_id= d.product_id;', function(err, rows, fields)
        {       
		    if (err)
                throw err;
	        else
            {
				...
				{
					//var pic = rows[i].Image.toString('base64');
					//var pic = "/images/tee_shirt.jpg"
					var q = rows[i].Available_Qty;
					if (q == 0)
					htmlStr+="<tr> <td> <input type='checkbox' name='addToCartCheck' value='" +rows[i].Product_ID +"' disabled='disabled' /> </td> <td>"  +rows[i].Product_ID +"</td> <td>"  + rows[i].Name + "</td> <td>" +rows[i].Price +"$" + "</td> <td> " +rows[i].Available_Qty +   "</td> <td> " +rows[i].Attr_Name +   "</td> <td> " +rows[i].Attr_Value +  " </td> <td > <img src=' "  + rows[i].Image_Path +  " ' style='width:100px;height:100px' /> </td> <td> <select name='purchaseQty' > <option value='1'>1</option><option value='2'>2</option><option value='3'>3</option></select> <td> </td></tr>";
					else
					htmlStr+="<tr> <td> <input type='checkbox' name='addToCartCheck' value='" +rows[i].Product_ID +"'/> </td> <td>"  +rows[i].Product_ID +"</td> <td>"  + rows[i].Name + "</td> <td>" +rows[i].Price +"$" + "</td> <td> " +rows[i].Available_Qty +   "</td> <td> " +rows[i].Attr_Name +   "</td> <td> " +rows[i].Attr_Value +  " </td> <td > <img src=' "  + rows[i].Image_Path +  " ' style='width:100px;height:100px' /> </td> <td> <select name='purchaseQty' > <option value='1'>1</option><option value='2'>2</option><option value='3'>3</option></select> <td> </td></tr>";
					//console.log(rows[i].Name);
                }
				
				
				res.writeHead(200, {"Content-Type": "text/html"});  
				res.write("<html> <head>  <link href='/css/cover.css' type='text/css' rel='stylesheet'> </head><body> <form  action ='/addProductsToCart' method= post> <table border='1'> <tr> <th> Select Product </th> <th> Product ID </th> <th> Product Name </th> <th> Price </th> <th> Available_Qty </th> <th> Attr_Name </th> <th> Attr_Value </th> <th> Product Image </th> <th> Purchase Quantity </th></tr>" +htmlStr +"<tr> <td colspan='9'> <input type='submit' name='add to cart' value='Add To Cart'/> </td> </tr> </table></form> </body> </html>"); 
				res.end(); 
            } 
	    });
	    ...
	    app.post("/proceedToPayment" , function(req,res){
			...
			mysql.query("select sum(tot_price)as amount from cart where UID = ?", [user_id], function(err, result, fields)
			{
				...
				{					
			        ...
			        mysql.query("Insert into ecommerce_project.order (UID,Order_Date,Purchase_Amt,Delivery_Addr_ID,Order_Status) VALUES ('"+order.UID+"', NOW(),'"+order.Purchase_Amt+"','"+order.Delivery_Addr_ID+"','"+order.Order_Status+"') ; ",function(err, result)
				    {
					    if (err)
						    throw err;
				        else
	            ...
	            mysql.query('SELECT * FROM customer where email_id= ? ', [email_id] , function(err,rows)
     	        {
	                ...
	                else
                    {
	                    uid=rows[0].uid; 
	                    console.log("uid" +uid);
	                    mysql.query("Insert into user_entry (user_name,pass_word,cid) values ('"+user_name+"', '"+pass_word+"','"+uid+"');", function(err, result)
	                    {
	                        if (err)
                                throw err;
	                        else
    ...
    app.get("/customer_loggedin",function(req,res){
	    res.sendfile("./customer_loggedin.html");
 	    ...
 	    app.get("/userOrderHistory",function(req,res){
            mysql.query('select b.Order_ID, b.Seq_num, b.Product_ID, b.QTY, b.Price, a.order_date, a.Order_Status ,b.Return_Status from ecommerce_project.order a, line_items b where (a.Order_ID=b.Order_ID) and (a.UID=?)',[user_id], function(err, rows, fields)
		{
            if (err)
                throw err;
            else
            {
                ...
                for(i=0;i<rows.length;i++)
                {
					...
					var lineitemid =  String(rows[i].Order_ID)+String(rows[i].Seq_num);
                    htmlStr+="<tr> <td> <input type='checkbox' name='returnItems' value='" +lineitemid +"'/> </td> <td>"  +rows[i].Order_ID +"</td> <td>"  + rows[i].Seq_num + "</td> <td>" +rows[i].Product_ID + "</td> <td> " +rows[i].QTY +   "</td> <td> " +rows[i].Price +   "</td> <td> " +rows[i].order_date + "</td> <td> " +rows[i].Order_Status +"</td> <td> " +rows[i].Return_Status + " </td> </tr>";
                    console.log(rows[i].Product_ID);
                }
                res.writeHead(200, {"Content-Type": "text/html"});
		...
		app.post("/customerReturns" , function(req,res)
	        {
                ...
                var temp = function(index)
                {
                    ...
                    else
                    {
                        ...
                        //console.log("selected check box now" +checked_product );
                        //console.log("check loop1");
			            var flag_yes = "Initiated" 


							var checked_order = checked[index];
							var checked_order_id = checked_order.slice(0,-1);
							var checked_seq_no = checked_order.slice(-1);
						
                        mysql.query("update line_items set Return_Status=? where Order_ID=? and Seq_num = ?", [flag_yes,checked_order_id,checked_seq_no], function(err, result, fields)
                    {
                        if (err)
                            throw err;
                        else
    ...
    app.get("/supervisor_loggedin",function(req,res){
	    ...
	    app.get("/viewOrders",function(req,res){
	        mysql.query('select b.Order_ID, b.Seq_num, b.Product_ID, b.QTY, b.Price, a.order_date, a.Order_Status ,b.Return_Status from ecommerce_project.order a, line_items b where a.Order_ID=b.Order_ID;', function(err, rows, fields)
            {       
		        if (err)
                    throw err;
	            else
                {
				    ...
				    for(i=0;i<rows.length;i++)
				    {
					    htmlStr+="<tr> <td>"  +rows[i].Order_ID +"</td> <td>"  + rows[i].Seq_num + "</td> <td>" +rows[i].Product_ID  + "</td> <td> " +rows[i].QTY +   "</td> <td> " +rows[i].Price +   "</td> <td> " +rows[i].order_date +  " </td> <td> " +rows[i].Order_Status+ " </td> <td> "  +rows[i].Return_Status+ " </td></tr>";	
					    console.log(rows[i].Name);			
				    }
				    res.writeHead(200, {"Content-Type": "text/html"});  
				    res.write("<html> <head>  <link href='/css/cover.css' type='text/css' rel='stylesheet'> </head><body> <form  action ='/addProductsToCart' method= post> <table border='1'> <tr> <th> Order_ID </th> <th> Seq_num </th> <th> Product_ID </th> <th> QTY </th> <th> Price </th> <th> order_date </th> <th> Order_Status </th> <th> Return_Status </th></tr>" +htmlStr +"</tr> </table></form> </body> </html>"); 
				    res.end();
                } 
	        });
	    });
		...
		app.get("/updateShipments",function(req,res){	
			...
			app.get("/ProcessReturn",function(req,res)
			{	
			mysql.query('select b.Order_ID, b.Seq_num, b.Product_ID, b.QTY, b.Price, a.order_date, a.Order_Status ,b.Return_Status from ecommerce_project.order a, line_items b where a.Order_ID=b.Order_ID;', function(err, rows, fields)
            {       
		        if (err)
	            ...
	            else
                {
				    ...
				    for(i=0;i<rows.length;i++)
				    {
						...
						var lineitemid =  String(rows[i].Order_ID)+String(rows[i].Seq_num); 
					    htmlStr+="<tr> <td> <input type='checkbox' name='returnUpdate' value='" +lineitemid+"'/> </td><td>"  +rows[i].Order_ID +"</td> <td>"  + rows[i].Seq_num + "</td> <td>" +rows[i].Product_ID  + "</td> <td> " +rows[i].QTY +   "</td> <td> " +rows[i].Price +   "</td> <td> " +rows[i].order_date +  " </td> <td> " +rows[i].Order_Status+ " </td> <td> "  +rows[i].Return_Status+ " </td></tr>";	
					    			
				    }
				    res.writeHead(200, {"Content-Type": "text/html"});  
				    res.write("<html> <head> <link href='/css/cover.css' type='text/css' rel='stylesheet'> </head><body> <form  action ='/SupervisorReturnProcess' method= post> <table border='1'> <tr> <th> Select </th> <th> Order_ID </th> <th> Seq_num </th> <th> Product_ID </th> <th> QTY </th> <th> Price </th> <th> order_date </th> <th> Order_Status </th> <th> Return_Status </th></tr>" +htmlStr +"</tr> <tr> <td colspan='5' > <input type='submit' name='returnAction' value='Approve' /> </td>  <td colspan='4' > <input type='submit' name='returnAction' value='Reject' /> </td>    </table></form> </body> </html>"); 
				    res.end();
                } 
	        });
				
				
			app.post("/SupervisorReturnProcess",function(req,res)
			{	
			var checked=req.body.returnUpdate;
			var action1 =req.body.returnAction
			console.log("SupervisorReturnProcess action =" +action1)
			if(action1 == "Approve")
			{	
				...
				var statusUpdate = function(index)
				{
					...
					else
					{
					    ...
					    console.log("selected check box now" +checked_order +"ID" +checked_order_id + "Seq num" +checked_seq_no);
						
					    mysql.query("update line_items set Return_Status = ? where Order_ID = ? and Seq_num = ? ", [Return_Status,checked_order_id,checked_seq_no],function(err, result, fields)
					    {										
					        if (err)		
						        throw err;
					        else
							{	
					        console.log("return status update to line items success");	
							
							mysql.query("select price from line_items where Order_ID = ? and Seq_num = ? " , [checked_order_id,checked_seq_no], function(err, result, fields)
							{	
							var checked_item_price = result[0].price; 
							var tot_order_price; 
							if (err)		
						        throw err;
					        else
							{		mysql.query("select Purchase_Amt from ecommerce_project.order where Order_ID = ?" , [checked_order_id], function(err, rows)
										{	
										tot_order_price =  rows[0].Purchase_Amt - checked_item_price;
										console.log("updated order amt = " +tot_order_price );
										if (err)		
											throw err;
										else
										{
												 mysql.query("update ecommerce_project.order set Purchase_Amt = ? where Order_ID = ? ", [tot_order_price,checked_order_id],function(err, result, fields)
													{										
														if (err)		
															throw err;
														else
														{
														 console.log("purchase amount updated successfully in order table")
														}
													});	
										}
										});
					...
					else
					{
					    ...
					    {										
					        if (err)		
					            ...
					            console.log("return status update success");								 				      
					    });
				        statusUpdate(index +1);	
					}	
			...
			res.send("Retruns processed successfully")
			});				
